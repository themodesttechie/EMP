        ...
        <div className="flex-[1.3] bg-white dark:bg-[#111] rounded-3xl border border-slate-200 dark:border-white/10 shadow-sm flex flex-col overflow-hidden">
             ...
             <div className="flex-none px-5 py-3 border-b border-slate-100 dark:border-white/5">
                <div className="grid grid-cols-7 gap-1">
                   {['M','T','W','T','F','S','S'].map(d => <div key={d} className="text-center text-[9px] font-bold text-slate-300 mb-1">{d}</div>)}
                   ...
                   filteredList.map(req => {
                      ...
                      return (
                          <div 
                            ...
                            className={`group cursor-pointer w-full p-3 rounded-2xl border transition-all relative overflow-hidden flex items-center gap-4 ${
                              isSelected 
                          ...
                          >
                            <div className={`flex-shrink-0 w-14 h-14 rounded-xl flex flex-col items-center justify-center border ${
                                isSelected ? "bg-white/10 border-white/10" : "bg-slate-50 dark:bg-white/5 border-slate-100 dark:border-white/5"
                            }`}>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-center mb-0.5">
                                  ...
                                  <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full border ${
                                      isSelected ? "bg-white/20 border-white/10 text-white" : getStatusStyle(req.status)
                                  }`}>
                                      {req.status}
                                  </span>
                                </div>
           ...
           <div className={`p-6 border-b border-slate-50 dark:border-white/5 transition-colors ${isEditing ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : 'bg-slate-50/50 dark:bg-white/[0.02]'}`}>
              <div className="flex justify-between items-start mb-6">
                 ...
                 <div className={`px-3 py-1.5 rounded-xl border flex items-center gap-2 ${
                    isEditing ? "bg-white border-indigo-200 text-indigo-600" : getStatusStyle(selectedReq.status)
                 }`}>
                    ...
                    <div className="relative pl-6 border-l-2 border-slate-100 dark:border-white/5 space-y-6">
                       {selectedReq.approvers.map((p, i) => (
                          <div key={i} className="relative">
                             <div className={`absolute -left-[29px] top-0 w-3 h-3 rounded-full border-2 border-white dark:border-[#111] z-10 ${
                                selectedReq.status === 'Approved' ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-white/20'
                             }`} />
